...
class LinkCheckPlugin_OLD implements EventSubscriberInterface
{
    protected $app;
    protected $output;
    protected $item;
    ...
    public static function getSubscribedEvents()
    {
        return array(
                ...
                Events::POST_PUBLISH => array('onPostPublish', -100) // after LinkPlugin
        );
    }
    ...
    protected function fixInternalLinks($content)
    {
        ...
        // undo changes done by LinkPlugin
        $content = preg_replace_callback(
                '/<a (?<prev>.*)href="\.\/(?<link>.*)" ?(?<post>.*)>(?<text>.*)<\/a>/Us',
                function ($matches)
                {
                    $link = sprintf(
                            '<a %s href="%s" %s>%s</a>',
    ...
    public function onPostPublish(BaseEvent $event)
    {
        ...
        $links = $this->findBookLinks($event);
    ...
    protected function findBookLinks(BaseEvent $event)
    {
        $links = array(
                'internal' => array(),
                'external' => array()
        ...
        foreach ($bookPages as $bookPage) {
            ...
            print_r($bookPage->getPathname());

            $matches = preg_match_all('/<a href="(?<uri>#.*)".*<\/a>/Us', $htmlContent);
