...
abstract class BookPublishingAllTestCase extends TestCase
{
    ...
    public function testBookPublish(
        ...
        // Read the book config file to get the edition options
        $bookConfig = Yaml::parse(file_get_contents($bookConfigFile));
        $editions = $bookConfig['book']['editions'];
        $edition = $editions[$editionName] ?? [];
        ...
        foreach ($generatedFiles as $file) {
            ...
            if ('epub' === $file->getExtension()) {
                ...
                // PDF files cannot be compared to expected results
                // because their contents are binary and always change,
                // so we just check that the file size is the same.

                // WeasyPrint generates different size PDF files each time (seems time dependent or random)
                // so we have to skip this assertion
                if ($edition['pdf_engine'] ?? null === 'weasyprint') {
                    $this->markTestSkipped('SKIPPED because the PDF file generated by WeasyPrint has a different size each time.');
                } else {
                    static::assertEquals(
                        filesize($thisBookDir . '/expected/' . $editionName . '/' . $file->getRelativePathname()),
    ...
    static function assertFileEquivalent(
        string $expected,
        string $actual,
        string $message = '',
